...
int initialize_collapse_times(int ismooth, int onlycompute)
{
  ...
  nnndgp = fopen( "nnndgp.out", "w" ); //nDGP
...
double linearmu_nDGP(double a)
{
  double H_over_c,nDGP_Hrc,Esq,Hdot_H02,beta;

  H_over_c=100.*params.Hubble100/299792.458;
  nDGP_Hrc=nDGP_rc*H_over_c;

  Esq=params.Omega0/a/a/a+params.OmegaLambda;
  Hdot_H02=-3.*params.Omega0/2./a/a/a;
  

  beta=1+2.*nDGP_Hrc*(1.+Hdot_H02/3./Esq)*sqrt(Esq);
...
#ifdef vain_nDGP
double vainmu_nDGP(double a,double delta)
{
  double H_over_c,nDGP_Hrc,Esq,Hdot_H02,OmegaMatter,beta,Rv_R3;
  ...
  Esq=params.Omega0/a/a/a+params.OmegaLambda;
  Hdot_H02=-3.*params.Omega0/2./a/a/a;
  OmegaMatter=params.Omega0/Esq/a/a/a;
  

  beta=1+2.*nDGP_Hrc*(1.+Hdot_H02/3./Esq)*sqrt(Esq);
  Rv_R3=8.*nDGP_Hrc*nDGP_Hrc*Esq*OmegaMatter*delta/beta/beta/9.;

  return 2.*(-1.+sqrt(1.+Rv_R3))/Rv_R3/3./beta;
...
#endif
#ifdef linear_nDGP
                     - 3.0*omegam*y[i+6] * (1. + linearmu_nDGP(t))
#endif
#ifdef vain_nDGP
                     - 3.0*omegam*y[i+6] * (1. + vainmu_nDGP(t,delta))
#endif
#if !defined(MOD_GRAV_FR) && (!defined(linearG3) && !defined(vainG3))
#if !defined(linear_nDGP) && !defined(vain_nDGP)
                     - 3.0*omegam*y[i+6] 
#endif
#endif
...
double ell_sng(int ismooth, double l1, double l2, double l3)
{
  ...
  while (mya<amax)
      ...
      if (status!=GSL_SUCCESS)
	{
      ...
      delta=y[6]+y[7]+y[8]; //nDGP
      if (y[0]>=0.99999)  
        { //fprintf(nnndgp,"%12lg   %12lg   %12lg\n",mya,delta,vainmu_nDGP(mya,delta)); //nDGP
